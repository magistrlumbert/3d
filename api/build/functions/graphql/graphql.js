...
var _index = require("@apollo/gateway/dist/index");
...
const {
  ApolloServer
} = require('apollo-server-lambda'); // This module is copied during the build step
// Be sure to run `npm run build`


const gateway = new _index.ApolloGateway({
  // This entire `serviceList` is optional when running in managed federation
  // mode, using Apollo Graph Manager as the source of truth.  In production,
  // using a single source of truth to compose a schema is recommended and
  // prevents composition failures at runtime using schema validation using
  // real usage-based metrics.
  serviceList: [// { name: 'bgi', url: process.env.NEO4J_URI_BGI },
  {
    name: 'bgi',
